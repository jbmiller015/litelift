...
export default function editExercise_old({weightsData = [], edit = false, key}: EditProps) {

    const [weights, setWeights] = useState(weightsData);
    const [editExercise_old, setEditExercise] = useState(edit);
    useEffect(() => {
        if (weights.length < 1) {
            addWeight();
        }
    ...
    return (
        <div className="" key={key}>
            <form className="flex flew-row flex-wrap max-w-sm mx-auto">
                <div>
                    <input type="text" id="exercise" aria-describedby="editExercise_old-name" placeholder="Exercise"
